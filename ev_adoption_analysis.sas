...
/* Code generated on: 06/02/2026, 17:55 */

%web_drop_table(WORK.IMPORT);


/* EV Adoption & Market Analysis
...
/* Optional quick view: top makes in the highest-adoption county */
PROC PRINT DATA=WORK.make_by_county_share (OBS=30);
    WHERE county = "King";
RUN;
...
/* Q4, PART 4: how do vehicle characteristics differ between high-adoption and low-adoption regions? (CAFV eligibility mix) */

PROC SQL;
    CREATE TABLE WORK.cafv_mix_by_group AS
    SELECT 
        adoption_group,
        "Clean Alternative Fuel Vehicle ("n AS cafv_status,
        COUNT(*) AS ev_count
    FROM (
        SELECT 
            e.*,
    ...
    GROUP BY adoption_group, "Clean Alternative Fuel Vehicle ("n
    ORDER BY adoption_group, ev_count DESC;
QUIT;

PROC PRINT DATA=WORK.cafv_mix_by_group;
RUN;
